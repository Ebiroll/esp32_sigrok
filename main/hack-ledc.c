...
{
    uint64_t clk_freq = 80000000; // getApbFrequency();
...
static double _ledcTimerRead(uint8_t chan)
{
    ...
    if(apb_clk) {
        clk_freq = 80000000; // getApbFrequency();
    }
    clk_freq <<= 8;//div_num is 8 bit decimal
    return (clk_freq >> bit_num) / (double)div_num;
}
...
void ledcAttachPin(uint8_t pin, uint8_t chan)
{
    ...
    gpio_matrix_out(pin, ((chan/8)? LEDC_LS_SIG_OUT0_IDX : LEDC_HS_SIG_OUT0_IDX) + (chan%8), false, false);
}
